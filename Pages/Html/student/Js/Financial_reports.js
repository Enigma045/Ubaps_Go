...
async function generatePDFReport() {
    ...
    try {
        ...
        const reportTitle = window.currentReportTitle || "Comprehensive Financial Report";
        ...
        doc.save(`UBAPS_Finance_Report_${yearLabel.replace(/\//g, '-')}.pdf`);
...
function updatePayloadAndFetch() {
    ...
    if (filter[3] && filter[3].checked) {
        statusList.push(filter[3].value);
        statusLabel = "Paid Students";
    }
...
async function fetchReportStats() {
    ...
    try {
        ...
        if (total > 0) {
            ...
            set('rate-paid', ((s.paid / total) * 100).toFixed(1) + '%');
